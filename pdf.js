			...
			$('.file')
				.each(
					function() {
						...
						ext : source.text().substr(source.text().lastIndexOf('.') + 1)
							.toLowerCase()
					    };
					    ...
					    switch (details.ext) {
					    case 'pdf':
						// TODO: convert the sizes to admin configurable settings... if anyone
						// asks.
						// use an <object> wrapping an <embed> for backwards compatibility
						// reasons.
						addElement(
							source,
							details,
						...
						break;
					    /*
					     * case 'html': // You might not want this.. $.ajax({ url : details.href,
					     * dataType : "html", success : function(data) { addElement(source, details,
					     * data); }, }); break;
					     */
					    default:
